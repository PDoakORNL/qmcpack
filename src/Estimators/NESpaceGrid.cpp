  ...
  bool NESpaceGrid::processAxis(SpaceGridInput& input)
{
    ...
    {
      app_log() << "  interval endparticles_ cannot be greater than " << 1 << std::endl;
  ...
  bool NESpaceGrid::initializeRectilinear(SpaceGridInput & input, Points & points_)
  {
    ...
    auto coord_form = input_.get_coordinate_form();
    ...
    if (origin_p1.size() > 0)
    {
      ...
      origin                 = points__[p1] + input_.get_origin_fraction() * (points_[origin_p2] - points_[origin_p1]);
    }
    else
      origin_ = points__["zero"];
    ...
    while (element != NULL)
    {
      ...
      ndomains_ = dimensions[0] * dimensions[1] * dimensions[2];
      volume    = std::abs(det(axes)) * 8.0; //axes span only one octant
      //compute domain volumes, centers, and widths
      domain_volumes.resize(ndomains_, 1);
      domain_centers.resize(ndomains_, DIM);
      domain_uwidths.resize(ndomains_, DIM);
      ...
      for (int d = 0; d < DIM; d++)
      {
        ...
        for (int i = 1; i < nintervals; i++)
        {
          ...
          interval_centers[d][i] =
              interval_centers[d][i - 1] + .5 * (interval_widths[d][i] + interval_widths[d][i - 1]);
        }
      ...
      if (chempot)
      {
        reference_count.resize(ndomains_);
    ...
    void NESpaceGrid::write_description(std::ostream & os, std::string & indent)
    {
      ...
      os << indent + "  ndomains      = " << ndomains_ << std::endl;
    ...
    int NESpaceGrid::allocate_buffer_space(BufferType & buf)
    {
      ...
      if (!chempot)
      {
        ...
        buffer_end   = buffer_start + nvalues_per_domain_ * ndomains_ - 1;
      }
      else
      {
        std::vector<Real> tmp(nvalues_per_domain_ * npvalues * ndomains_);
        buf.add(tmp.begin(), tmp.end());
        buffer_start = buffer_offset;
        buffer_end   = buffer_start + nvalues_per_domain_ * npvalues * ndomains_ - 1;
      }
    ...
    void NESpaceGrid::registerCollectables(std::vector<ObservableHelper> & h5desc, hid_t gid, int grid_index) const
    {
      ...
      if (!chempot)
        ...
        ng[0] = nvalues_per_domain_ * npvalues * ndomains_;
...
#define NESpaceGrid_CHECK
    ...
    void NESpaceGrid::evaluate(const ParticlePos& R, const Matrix<Real>& values, BufferType& buf,
                               std::vector<bool>& particles_outside, const DistanceTableAB& dtab)
    {
      int p, v;
      ...
      if (!chempot)
      {
        ...
        case cartesian:
          ...
          else
          {
            ...
            {
              u = dot(axinv, (R[p] - origin));
              if (u[0] > umin[0] && u[0] < umax[0] && u[1] > umin[1] && u[1] < umax[1] && u[2] > umin[2] &&
                  u[2] < umax[2])
              {
                particles_outside[p] = false;
