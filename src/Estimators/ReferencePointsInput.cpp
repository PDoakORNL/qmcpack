...
bool ReferencePointsInput::readXML(xmlNodePtr cur)
{
  ...
  std::string node_contents = XMLNodeString(cur);
  std::string_view stripped_contents  = strip(node_contents);
  std::vector<std::string_view> lines = split(stripped_contents, "\n");
  //remove empty lines
  std::remove_if(lines.begin(), lines.end(), [](auto line) -> bool { return strip(line) == std::string_view{}; });
  for (int i = 0; i < lines.size(); i++)
  {
    succeeded                            = true;
    std::vector<std::string_view> tokens = split(strip(lines[i]), " \t");
    if (tokens.size() != OHMMS_DIM + 1)
    {
      app_log() << "  reference point must have 4 entries, given " << tokens.size() << ": " << lines[i] << std::endl;
      succeeded = false;
      break;
    }
    else
