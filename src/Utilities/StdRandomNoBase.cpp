...
{
template<typename T>
StdRandomNoBase<T>::StdRandomNoBase(uint_type iseed) : engine(iseed)
{
...
template<typename T>
void StdRandomNoBase<T>::load(const std::vector<uint_type>& newstate)
{
  std::stringstream otemp;
...
template<typename T>
void StdRandomNoBase<T>::save(std::vector<uint_type>& curstate) const
{
  curstate.clear();
  std::stringstream otemp;
  otemp << engine;
...
template<typename T>
typename StdRandomNoBase<T>::result_type StdRandomNoBase<T>::operator()()
{
  return distribution(engine);
}

template class StdRandomNoBase<double>;
template class StdRandomNoBase<float>;
} // namespace qmcplusplus
