...
struct h5data_proxy<Vector<T>> : public h5_space_type<T, 1>
{
  ...
  {
    std::array<hsize_t, FileSpace::rank> my_dims;
    my_dims[0]    = ref.size();
    int dim_index = 1;
    for (auto dim : dims)
    {
      my_dims[dim_index] = dims[dim_index];
      ++dim_index;
    }
    int index{1};
    return h5d_append(grp, aname.c_str(), current_append_index, FileSpace::rank, my_dims.data(),
