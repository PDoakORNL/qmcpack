                                              ...
                                              const std::vector<ListenerVector<RealType>>& listeners_ions) const
{
  ...
  auto evaluate_walker = [name, &ve_sample, &vi_sample, myTableIndex, NumIons,
                          Zeff](int walker_index, const ParticleSet& pset, const auto& PP,
                                const std::vector<ListenerVector<RealType>>& listeners,
                                const std::vector<ListenerVector<RealType>>& listeners_ions) -> Return_t {
    ...
    for (const ListenerVector<RealType>& ion_listener : listeners_ions)
  ...
  for (int iw = 0; iw < o_list.size(); ++iw)
  {
    ...
    local_ecp.value_ = evaluate_walker(iw, p_list[iw], PP, listeners, listeners_ions);
  }
}
